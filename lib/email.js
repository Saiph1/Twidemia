...
async function sendToken({token}) {
  ...
  if (token.type == "forgot") {
    content = `
      <p> Hello user: </p>
      <p>To reset your password, please follow this link: <a target="_" href=\"${process.env.DOMAIN}/reset/${token.hash}\"> ${process.env.DOMAIN}/reset/${token.hash} </a></p>
      <p>Cheers</p>
      <p>Twidemia Team</p>
    `
  }
  ...
  let info = await transporter.sendMail({
    ...
    subject: "Email Verification for Twidemia", // Subject line
